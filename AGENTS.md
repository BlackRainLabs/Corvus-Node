...
7. **Isolation**:
   ...
   - CLI/GUI talk to Node on host AF_UNIX (`0660`, group `corvus`). The Node **service** needs jailer/KVM/root. The operator CLI does not. `vm` / `chat` fail closed if Node is not running (`corvus start` after an install; `./install.sh` if Corvus-Node was never set up).
